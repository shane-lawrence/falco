...
k8s_audit_filter_check::k8s_audit_filter_check()
{
	m_info = {"ka",
		  "Access K8s Audit Log Events",
		  {
			  ...
			  {"ka.req.container.allowPrivilegeEscalation", "When the request object refers to a container, whether or not any container has allowPrivilegeEscalation=true. With an index, return whether or not the ith container has allowPrivilegeEscalation=true."},
			  ...
			  {"ka.req.container.runAsUser.defined", "When the request object refers to a container, return true if all containers specify a runAsUser. With an index, return whether or not the ith container specifies a runAsUser."},
			  ...
			  {"ka.req.container.runAsGroup.defined", "When the request object refers to a container, return true if all containers specify a runAsGroup. With an index, return whether or not the ith container specifies a runAsGroup."},
			  ...
			  {"ka.req.sec_ctx.allowedCapabilities.within", "When the request object refers to a pod, whether the set of allowed capabilities is within the provided list. For example, ka.req.sec_ctx.allowedCapabilities.within[CAP_KILL] would only allow pods to add the CAP_KILL capability and no other capability"},
...

	{
		...
		m_aliases = {
			...
			{"ka.req.container.allowPrivilegeEscalation", {"/requestObject/spec/containers"_json_pointer, index_allow_privilege_escalation, a::IDX_ALLOWED, a::IDX_NUMERIC}},
			...
			{"ka.req.sec_ctx.allowedCapabilities.within", {"/requestObject/spec/securityContext/allowedCapabilities"_json_pointer, check_allowed_capabilities, a::IDX_REQUIRED, a::IDX_KEY}},
