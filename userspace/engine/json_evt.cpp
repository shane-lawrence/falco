...
k8s_audit_filter_check::k8s_audit_filter_check()
{
	m_info = {"ka",
		  "Access K8s Audit Log Events",
		  {
			  ...
			  {"ka.req.container.proc_mount.within", "When the request object refers to a container, whether the procMount types specified for all containers are within the provided set. Example: ka.req.container.proc_mount.within[Unmasked,Default] returns true if all containers' procMount values are \"Unmasked\" or \"Default\"."},
...

	{
		...
		m_aliases = {
			...
			{"ka.req.container.proc_mount.within", {"/requestObject/spec/containers"_json_pointer, check_proc_mount_within, a::IDX_ALLOWED, a::IDX_KEY}},
			...
			{"ka.req.sec_ctx.added_capabilities.within", {"/requestObject/spec/containers"_json_pointer, check_added_capabilities, a::IDX_REQUIRED, a::IDX_KEY}},
